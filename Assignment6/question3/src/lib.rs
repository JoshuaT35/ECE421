...
impl UserBase {
    ...
    pub fn add_user(&self, u_name: &str, p_word: &str) -> Result<(), UBaseErr> {
        ...
        let hpass = argon2::hash_encoded(p_word.as_bytes(), salt, &config).unwrap();
    ...
    // verify user exists
    pub fn verify_user(&self, u_name: &str, p_word: &str) -> Result<bool, UBaseErr> {
        // open connection to the database
        let conn = sqlite::open(&self.fname)?;
        conn.execute("PRAGMA foreign_keys = ON;")?;
        ...
        let mut stmt = conn.prepare(
            "SELECT p_word FROM users WHERE u_name = :user;"
        )?;
        stmt.bind((":user", u_name))?;

        // move to the first row
        if stmt.next()? == sqlite::State::Row {
            let p_word_hash: String = stmt.read::<String, _>(0)?;

            // verify the password hash
            let valid_password = argon2::verify_encoded(&p_word_hash, p_word.as_bytes())?;
            return Ok(valid_password);
        }
